...
from app.models import (
    Agent, AgentState, AgentRole, Task, TaskStep, LogEntry,
    TaskStrategy, TaskStatus, StepStatus, LogLevel,
    AgentMessage, MessageType, MessagePriority, TaskChatMessage,
)
from app.services.llm_client import chat_completion
from app.services.logger import log_broadcaster
from app.services.crash_recovery import crash_recovery
...
async def _log_agent_message(task_id: int, agent: Agent, message: str, message_type: str = "agent_message"):
    """Log agent-to-agent communication message."""
    async with async_session() as msg_session:
        # Log to task chat messages
        chat_msg = TaskChatMessage(
            task_id=task_id,
            sender_id=agent.id,
            message=message,
            message_type=message_type,
        )
        msg_session.add(chat_msg)
        
        # Also log to agent messages for project-wide visibility
        agent_msg = AgentMessage(
            task_id=task_id,
            sender_id=agent.id,
            message_type=MessageType(message_type) if message_type in [e.value for e in MessageType] else MessageType.STATUS_REPORT,
            priority=MessagePriority.NORMAL,
            payload={"message": message, "agent_name": agent.name, "agent_role": agent.role.value},
        )
        msg_session.add(agent_msg)
...
async def _run_step(
    ...
    async with async_session() as step_session:
        ...
        try:
            ...
            # Use crash recovery wrapper to ensure data persistence
            effective_agent = local_agent or agent
            response_text, duration_ms, error = await crash_recovery.execute_with_recovery(
                agent=effective_agent,
                task_step_id=step.id,
                execution_func=lambda: chat_completion(
                    agent=effective_agent,
            ...
            # Track performance
            performance_tracker.record_response(agent.id, duration_ms, error is None)
            ...
            if error:
                ...
                await _log(task_id, f"[{agent.name}] completed with error: {error}", source=agent.name, level="warning")
            else:
            ...
            # Log agent message for chat visibility
            if not error and response_text:
                # Truncate long responses for chat
                chat_message = response_text[:500] + "..." if len(response_text) > 500 else response_text
                await _log_agent_message(
                    task_id=task_id,
                    agent=agent,
                    message=f"Completed step '{step_label or 'task'}': {chat_message}",
...
async def execute_sequential(session: AsyncSession, task: Task, agents: list[Agent]):
    ...
    if agents:
        await _log_agent_message(
            task_id=task.id,
            agent=agents[0],
            message=f"Starting sequential execution with {len(agents)} agents in chain",
...
async def execute_parallel(session: AsyncSession, task: Task, agents: list[Agent]):
    ...
    if agents:
        await _log_agent_message(
            task_id=task.id,
            agent=agents[0],
            message=f"Starting parallel execution - fanning out to {len(agents)} agents concurrently",
            message_type="notification",
        )
...
async def execute_hierarchical(session: AsyncSession, task: Task, agents: list[Agent]):
    """
    Hierarchical strategy: CEO receives requirements, selects planning agents,
    approves the plan, assigns roles, and routes execution to managers/employees.
    """
    await _log(task.id, "🧩 HIERARCHICAL execution begins — CEO-led command chain")

    ceo_agent = next((a for a in agents if a.is_orchestrator), None)
    if not ceo_agent:
        ceo_agent = next((a for a in agents if a.role == AgentRole.CEO), None)

    if not ceo_agent:
        await _log(task.id, "No CEO/orchestrator found — falling back to parallel", level="warning")
        return await execute_parallel(session, task, agents)

    level05_agents = [a for a in agents if a.role in (AgentRole.CTO, AgentRole.MANAGER, AgentRole.EMPLOYEE) and a.is_active]
    if not level05_agents:
        task.status = TaskStatus.FAILED
        task.final_output = "No available planning agents for hierarchical workflow."
        await _log(task.id, "No planning agents available for hierarchical workflow", level="error")
        return

    await _log(task.id, f"CEO {ceo_agent.name} selected {len(level05_agents)} planning agent(s)")

    planning_outputs = []
    for i, planner in enumerate(level05_agents):
        prompt = (
            f"You are {planner.name} ({planner.role.value.upper()}).\n"
            f"The CEO asked: {task.prompt}\n"
            "Please produce a Level 0.5 architecture and workforce estimate for implementation."
        )
        result = await _run_step(task.id, planner, prompt, step_order=i, step_label="hierarchical_planning")
        if result["status"] == StepStatus.COMPLETED:
            planning_outputs.append((planner, result["response"]))

    if not planning_outputs:
        task.status = TaskStatus.FAILED
        task.final_output = "All planning agents failed to generate a plan."
        await _log(task.id, "Hierarchical planning failed — no outputs", level="error")
        return

    await _log(task.id, f"Hierarchical planning complete — {len(planning_outputs)} outputs collected")

    review_prompt = (
        f"You are CEO {ceo_agent.name}. Review the following Level 0.5 plans and approve or reject:\n\n"
        + "\n\n".join(
            f"--- {planner.name} ({planner.role.value}) ---\n{response}"
            for planner, response in planning_outputs
        )
        + "\n\nRespond with: APPROVED or REJECTED and a short rationale."
    )

    review_result = await _run_step(task.id, ceo_agent, review_prompt, step_order=len(level05_agents), step_label="ceo_review")
    if review_result["status"] != StepStatus.COMPLETED:
        task.status = TaskStatus.FAILED
        task.final_output = "CEO review failed."
        await _log(task.id, "Hierarchical CEO review failed", level="error")
        return

    review_text = review_result["response"].lower()
    if "reject" in review_text and "approved" not in review_text:
        approved = False
    else:
        approved = True

    await _log(task.id, f"CEO review result: {'approved' if approved else 'rejected'}")

    if not approved:
        task.status = TaskStatus.FAILED
        task.final_output = "CEO rejected the hierarchical plan.\n" + review_result["response"]
        return

    # Assign roles from CEO strategy if available, otherwise round robin by role
    for i, (planner, _) in enumerate(planning_outputs):
        if i == 0:
            planner.role = AgentRole.CTO
        elif i == 1:
            planner.role = AgentRole.MANAGER
        else:
            planner.role = AgentRole.EMPLOYEE

    # Execute delegation to operational agents
    execution_candidates = [a for a in agents if a.role in (AgentRole.MANAGER, AgentRole.EMPLOYEE)]
    execution_results = []
    for i, executor in enumerate(execution_candidates):
        exec_prompt = (
            f"You are {executor.name} ({executor.role.value.upper()}).\n"
            f"Based on the approved CEO plan, perform implementation planning for your assignment.\n"
            f"Original prompt: {task.prompt}\n"
            f"Summary: {planning_outputs[i % len(planning_outputs)][1]}"
        )
        result = await _run_step(task.id, executor, exec_prompt, step_order=len(level05_agents) + i + 1, step_label="hierarchical_execution")
        if result["status"] == StepStatus.COMPLETED:
            execution_results.append((executor, result["response"]))

    task.final_output = (
        "<HIERARCHICAL WORKFLOW COMPLETE>\n\n"
        f"CEO review: {review_result['response']}\n\n"
        "Planning outputs:\n" + "\n\n".join(f"{p.name}({p.role.value}): {o}" for p,o in planning_outputs) + "\n\n"
        "Execution outputs:\n" + "\n\n".join(f"{e.name}({e.role.value}): {o}" for e,o in execution_results)
    )
    task.status = TaskStatus.COMPLETED
    await _log(task.id, "✅ HIERARCHICAL workflow completed successfully")
...
async def execute_task(session: AsyncSession, task: Task, agents: list[Agent]):
    ...
    try:
        ...
        elif task.strategy == TaskStrategy.HIERARCHICAL:
            await execute_hierarchical(session, task, agents)
        else:
            task.status = TaskStatus.FAILED
